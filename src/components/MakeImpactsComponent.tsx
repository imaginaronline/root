...
import ImpactCardComponent from './ImpactMakeCardComponent';
...
function MakeImpactComponent() {
   const [events, setEvents] = useState<typeSelection[] | []>([]);
    ...
    const url = `${API_ENDPOINTS.getAllDonationEvents}?SkipCount=${skipCount}&MaxResultCount=${maxResultCount}`;

    useEffect(() => {
        _proxy.get(url)
            .then((resp: any) => {
                // console.log("make an impact!!");
                var data = resp.data.data;
                setEvents(data); // Set event data in the state
            })
            .catch((error: any) => {
                console.log(error);
            });
    ...
    const impactDonationsResp = (resp: any) => {
    ...
    return (
        <div id="sectionToNavigate">
            <CarausolComponent itemsCount={3}>
                {events.map((event: any, index: any) => (
                    <div className="item" key={index}>
                        <ImpactCardComponent event={event} impactDonationsResp={impactDonationsResp} />
                    </div>
                ))}
            </CarausolComponent>
...
export default MakeImpactComponent